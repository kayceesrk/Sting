//$ bin/sessionjc -cp tests/classes/ tests/src/oopsla/schat/client/gui/ChatClientGui.sj -d tests/classes/
...
{		
	...
	public void createTab(Integer key, String title)
	{
		synchronized (mainPane)
		{
			...
			if (tab != null && tab....) // FIXME: if a private conversation is started with different client with the same ID as an old private conversation, change the tab title (to the new user name).
			{
