...
package aplas.bmarks2.micro.te.thread.server;
...
public class Server extends aplas.bmarks2.micro.Server {
	public protocol pBody rec X [?{REC: ?(ClientMessage).!<ServerMessage>.?(ClientMessage).?{L1: !<ServerMessage>. #X,L2: !<ServerMessage>.#X}, QUIT: }]
			...
			s.recursion(X) {
				s.inbranch() {
					case REC: {
						...
						s.send(new ServerMessage(cm.getServerMessageSize(),cm.getNum() + 2, server.kill));

						ClientMessage cm2 = (ClientMessage) s.receive();

						s.inbranch() {
							case L1: {
								s.send(new ServerMessage(cm.getServerMessageSize(),cm.getNum() + cm2.getNum(), server.kill));
