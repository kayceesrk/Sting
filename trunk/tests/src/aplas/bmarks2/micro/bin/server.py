#!/usr/bin/python
#
#tests/src/aplas/bmarks2/micro/bin/server.py <debug> <env> <server_port> <worker_port> <client_port> <version> <repeats>
#tests/src/aplas/bmarks2/micro/bin/server.py f localhost 8888 7777 6666 JT 2
		...
		for size in messageSizes:
			for length in sessionLengths:
				for i in range(0, repeats):
					...
					command = command + ' ' + transport + '-cp tests/classes aplas.bmarks2.micro.ServerRunner ' + str(debug) + ' ' + sport + ' ' + v + ' ' + str(len(workers))
