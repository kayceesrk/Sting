//$ bin/sessionj -cp tests/classes/ ecoop.bmarks.java.event.server.Server2 false 8888 1
        ...
        if (key.isValid())
        {
          ...
          {	            
          	handleRead(sel, key);	          
					...
					{
						throw new RuntimeException("[Server2] Unexpected key type: " + key);
					}	          
        }
      }
    } 
  ...
  private void prepareSend(Selector sel, SelectionKey key, MyObject mo) throws Exception
  {
    ...
    Attachment a = (Attachment) key.attachment();		
		...
		if (numRead == -1)
		{
			...
			throw new RuntimeException("[Server2] Connection failed: " + key);
		}
	}
	...
	private /*static */void write(SelectionKey key) throws IOException
	{
      ...
      if (counting) 
      {
        ...
        debugPrintln("[Server2] Current count:" + count); // This is the only non-static part of this method.		            
      }
			...
			while (updates.hasNext()) // Assumes that we won't change the interest op. more than once per channel per iteration of selector loop (we don't because protocol is lockstep receive and send).
			{
				...
				switch (update.getAction())
				{
					case KeyUpdate.CHANGE_OPS:
					{
						...
						if (key != null) // Client can fail in between sending something to Server2 and when Server2 gets around to processing the event?
						{
					...
					default:
					{
						throw new RuntimeException("[Server2] Unexpected key update action: " + update.getAction());
					}
				}
			}
