...
public class SJGProtocolDeclTranslator extends ContextVisitor // Subsequent ContextVisitor passes are broken because type building not done for new nodes? // Recording a reference to the containing AST node inside typeable instance objects might be useful for translation, maybe no need to manually track context scopes.
{
	...
	public SJGProtocolDeclTranslator(Job job, TypeSystem ts, NodeFactory nf)
	{
		super(job, ts, nf);
	...
	{
		if (n instanceof SJGProtocolDecl)
		{
			n = translateSJGProtocolDecl((SJGProtocolDecl) n);
		}
		
		return n;
	}

	private SJGProtocolDecl translateSJGProtocolDecl(SJGProtocolDecl pd) throws SemanticException
	{
		...
		{
			pd = ((SJFieldGProtocolDecl) pd).init(n);
		}
		else //if (pd instanceof SJLocalGProtocolDecl)
		{
			pd = ((SJLocalGProtocolDecl) pd).init(n);
		}
