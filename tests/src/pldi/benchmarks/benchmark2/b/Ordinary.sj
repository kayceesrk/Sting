...
//$ bin/sessionj -cp tests/classes/ pldi.benchmarks.benchmark2.b.Ordinary false 8888 0 1
//$ tests/src/pldi/benchmarks/benchmark2/b/ordinary.sh 8888 0

package pldi.benchmarks.benchmark2.b;
	...
	private void run(final boolean debug, final int port, final int depth, final int repeat) throws Exception
	{
		new Thread()
		{
			public void run()
			{
				try
				{
					new pldi.benchmarks.benchmark2.d.Server(port);
				}
				catch (Exception x)
				{
			...
			for (int li = 0; li < lens.length; li++)
			{
				...
				Thread t = new Thread()
				{
					public void run()
					{
						try
						{
							new pldi.benchmarks.benchmark1.a.Client().run(debug, "localhost", port, depth, len, 1);
						}
						catch (Exception x)
						{
							throw new RuntimeException(x);
